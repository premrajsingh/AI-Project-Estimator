...
class OrchestratorAgent:
    ...
    async def run_pipeline(
        ...
        try:
            ...
            if has_description:
                ...
                design_task  = asyncio.to_thread(self._run_design_validator, description)
            ...
            # ── Stage 2: SEQUENTIAL ──────────────────────────────────────────
            await asyncio.sleep(5)  # Increased for rate limit mitigation
            self._log("Stage 2a — Estimation")
            estimation_agent = EstimationAgent()
            estimations = estimation_agent.predict(metrics, hourly_rate=hourly_rate, num_developers=num_developers)
            ...
            if code and code.strip():
                ...
                code_result = reviewer.review(code)
            else:
                # Fallback: suggestion-mode from metrics
                reviewer = CodeReviewAgent()
                code_result = {"optimizations": reviewer.suggest(metrics, risks)}
    ...
    def _run_design_validator(self, description: str):
        agent = DesignValidatorAgent()
        return agent.validate(description, expected_days=30, team_size=3)
