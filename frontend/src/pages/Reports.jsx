...
import { FileText, ExternalLink, Calendar, Clock, IndianRupee, Lightbulb } from 'lucide-react';
import { Link } from 'react-router-dom';
import { getAllProjects, getAllPlannings } from '../services/api';
...
const Reports = () => {
  ...
  const allReports = [
    ...
    ...completedPlannings.map(p => ({
      ...
      link: null, // planning results viewed inline on /planning
    })),
          ...
          <div className="divide-y divide-slate-200 dark:divide-slate-800">
            {filtered.map((r) => (
              <div key={r.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                ...
                {r.link && (
                  <Link to={r.link} className="text-indigo-400 hover:text-indigo-300 transition-colors flex items-center gap-1 text-xs font-medium ml-2">
