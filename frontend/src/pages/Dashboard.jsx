...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setUrlError(null);
    try {
      const data = await analyzeProject(url || null, file, Math.floor(dailyRate / 8), numDevelopers);
              ...
              <div className="flex-1 w-full max-w-md">
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><Users className="w-3 h-3" /> No. of Developers</label>
                      <input 
                        type="number" 
                        min="1"
                        max="50"
                        value={numDevelopers}
                        onChange={(e) => setNumDevelopers(parseInt(e.target.value) || 1)}
                        className="w-full bg-slate-50 dark:bg-[#0B1120] border border-slate-300 dark:border-slate-700 rounded-lg py-2 px-3 text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:border-cyan-500/50 font-mono"
                        placeholder="1"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5"><IndianRupee className="w-3 h-3" /> Daily Rate (₹)</label>
                      <input 
                        type="number" 
                        min="100"
                        value={dailyRate}
